...
async def start_client(scheduler_addr, train_dir, model_file, num_workers):
  async with Client(scheduler_addr, asynchronous=True) as client:
    ...
    df = dask_cudf.read_csv([train_dir+t for t in os.listdir(train_dir)], header=None, names=colnames, chunksize=None)
    ...
    output = await xgb.dask.train(client,
                        ...
                        num_boost_round=100, evals=[(dtrain, 'train')])
    print("[debug:leader]: ------ training finished")
    output['booster'].save_model(model_file + '2')
    history = output['history']
    print('[debug:leader]: ------ Training evaluation history:', history)
    print("[debug:leader]: ------model saved")
...
if __name__=='__main__':
  ...
  parser.add_argument(
    ...
    to retrain""",
    default='gs://dlvm-dataset/models/001.model')
  parser.add_argument(
    '--num-workers',
    type=int,
  ...
  if task_name == 'master':
    ...
    with open(args.scheduler_ip_file, 'w') as f:
      f.write(host_ip)
    ...
    asyncio.get_event_loop().run_until_complete(start_client(scheduler_addr,
                             ...
                             args.model_file,
                             args.num_workers))
  # os.remove(args.scheduler_ip_file)
  # launch dask worker, redirect output to sys stdout/err
  elif task_name == 'worker':
    while not os.path.exists(args.scheduler_ip_file):
      time.sleep(1)
    with open(args.scheduler_ip_file, 'r') as f:
